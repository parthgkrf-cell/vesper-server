...
let state = {
  ...
  settings: {
    ...
    brevoKey: "",
    brevoEmail: "",
    brevoName: "Personal Sender",
    resendKey: "",
    resendEmail: "",
    localUrl: "http://localhost:3000/send",
    localAction: "send",
...
function loadStateFromCache() {
  ...
  if (cachedSettings) {
    try {
      ...
      // Upgrade method from legacy 'localserver' to specific send/draft method
      if (state.settings.method === "localserver") {
...
function initializeUI() {
  // Populate settings
  document.getElementById("brevo-api-key").value = state.settings.brevoKey;
  document.getElementById("brevo-sender-email").value = state.settings.brevoEmail;
  document.getElementById("brevo-sender-name").value = state.settings.brevoName;
  document.getElementById("resend-api-key").value = state.settings.resendKey;
  document.getElementById("resend-sender-email").value = state.settings.resendEmail;
...
function setupEventListeners() {
  ...
  const settingsInputs = [
    { id: "brevo-api-key", key: "brevoKey" },
    { id: "brevo-sender-email", key: "brevoEmail" },
    { id: "brevo-sender-name", key: "brevoName" },
    { id: "resend-api-key", key: "resendKey" },
    { id: "resend-sender-email", key: "resendEmail" },
...
function updateMethodFieldsVisibility(method) {
  document.getElementById("fields-brevo").classList.add("hidden");
  document.getElementById("fields-resend").classList.add("hidden");
  document.getElementById("fields-localserver").classList.add("hidden");

  if (method === "brevo") {
    document.getElementById("fields-brevo").classList.remove("hidden");
  } else if (method === "resend") {
    document.getElementById("fields-resend").classList.remove("hidden");
  } else if (method === "localserver" || method === "localserver_send" || method === "localserver_draft") {
    document.getElementById("fields-localserver").classList.remove("hidden");
    testLocalServerConnection();
  }
}
...
function updateLivePreview() {
  ...
  if (state.settings.method === "mailto") {
    document.getElementById("preview-sender-display").innerHTML = `Personal Client Mailto`;
  } else if (state.settings.method === "gmailweb") {
    document.getElementById("preview-sender-display").innerHTML = `Gmail Web Client (Draft)`;
  } else if (state.settings.method === "brevo") {
    document.getElementById("preview-sender-display").innerHTML = `${escapeHtml(state.settings.brevoName || 'Vesper Sender')} &lt;${escapeHtml(state.settings.brevoEmail || 'your-sender@domain.com')}&gt;`;
  } else if (state.settings.method === "resend") {
    document.getElementById("preview-sender-display").innerHTML = `Sender &lt;${escapeHtml(state.settings.resendEmail || 'your-domain@resend.dev')}&gt;`;
  } else {
...
function dispatchEmail(rec) {
  ...
  if (state.settings.method === "mailto") {
    ...
    try {
      ...
      if (rec.cc) {
        mailtoUrl += `&cc=${encodeURIComponent(rec.cc)}`;
      }
      if (rec.bcc) {
        mailtoUrl += `&bcc=${encodeURIComponent(rec.bcc)}`;
      }
      
      // Open in a new tab/window or trigger system redirect
      window.open(mailtoUrl, "_blank");
      
      rec.status = "Sent";
      addLog(`[Mailto] Triggered email compose dialog for ${rec.name} (${rec.email}).`, "success");
      showToast(`Mail client opened for ${rec.name}`, "success");
    } catch (err) {
      rec.status = "Failed";
      rec.error = err.message;
      addLog(`[Mailto] Error triggering client: ${err.message}`, "error");
    }
    updateRecStatusUI(rec);
    updateSummaryStats();
    saveRecipientsToCache();
    ...
    try {
      ...
      if (rec.cc) {
        gmailUrl += `&cc=${encodeURIComponent(rec.cc)}`;
      }
      if (rec.bcc) {
        gmailUrl += `&bcc=${encodeURIComponent(rec.bcc)}`;
      }
      
      window.open(gmailUrl, "_blank");
      
      rec.status = "Sent";
      addLog(`[Gmail Web] Triggered compose dialog for ${rec.name} (${rec.email}).`, "success");
      showToast(`Gmail opened for ${rec.name}`, "success");
    } catch (err) {
      rec.status = "Failed";
      rec.error = err.message;
      addLog(`[Gmail Web] Error triggering: ${err.message}`, "error");
    }
    updateRecStatusUI(rec);
    updateSummaryStats();
    saveRecipientsToCache();
    ...
    // Send API Brevo
    if (!state.settings.brevoKey) {
      failDispatch(rec, "Brevo API Key is missing in Configuration.");
      return;
    }
    
    addLog(`[Brevo API] Dispatched sending sequence to ${rec.name}...`, "system");
    
    const brevoPayload = {
      sender: { 
        name: state.settings.brevoName || "Personal Sender", 
        email: state.settings.brevoEmail 
      },
      to: [{ 
        email: rec.email, 
        name: rec.name 
    ...
    if (rec.cc) {
      brevoPayload.cc = rec.cc.split(',').map(e => ({ email: e.trim() }));
    }
    if (rec.bcc) {
      brevoPayload.bcc = rec.bcc.split(',').map(e => ({ email: e.trim() }));
    }
    ...
    fetch("https://api.brevo.com/v3/smtp/email", {
      ...
      headers: {
        ...
        "api-key": state.settings.brevoKey,
    ...
    .then(async (response) => {
      if (response.ok) {
        rec.status = "Sent";
        addLog(`[Brevo API] Sent email successfully to ${rec.name} (${rec.email})`, "success");
        updateRecStatusUI(rec);
        updateSummaryStats();
        saveRecipientsToCache();
      } else {
        const errText = await response.text();
        failDispatch(rec, `Brevo API Response code ${response.status}: ${errText}`);
      }
    })
    .catch((err) => {
      failDispatch(rec, `Fetch Exception: ${err.message}`);
    });

  } else if (state.settings.method === "resend") {
    // Send API Resend
    if (!state.settings.resendKey) {
      failDispatch(rec, "Resend API Key is missing in Configuration.");
      return;
    }

    addLog(`[Resend API] Dispatched sending sequence to ${rec.name}...`, "system");
    ...
    if (rec.cc) {
      resendPayload.cc = rec.cc.split(',').map(e => e.trim());
    }
    if (rec.bcc) {
      resendPayload.bcc = rec.bcc.split(',').map(e => e.trim());
    }

    fetch("https://api.resend.com/emails", {
      method: "POST",
      headers: {
        "Authorization": `Bearer ${state.settings.resendKey}`,
        "Content-Type": "application/json"
      },
    ...
    .then(async (response) => {
      if (response.ok) {
        rec.status = "Sent";
        addLog(`[Resend API] Sent email successfully to ${rec.name} (${rec.email})`, "success");
        updateRecStatusUI(rec);
        updateSummaryStats();
        saveRecipientsToCache();
      } else {
        const errText = await response.text();
        failDispatch(rec, `Resend API Response code ${response.status}: ${errText}`);
      }
    })
    .catch((err) => {
      failDispatch(rec, `Fetch Exception: ${err.message}`);
  ...
  } else if (state.settings.method === "localserver" || state.settings.method === "localserver_send" || state.settings.method === "localserver_draft") {
...
function sendTestEmail() {
  ...
  // Create temporary recipient mock
  const mockRec = {
    id: "test-rec-temp",
    name: "Test Recipient",
    email: testEmail,
    affiliation: "Testing Center",
    parsedTime: new Date(),
    status: "Pending"
  };
  ...
  if (state.settings.method === "mailto") {
    let mailtoUrl = `mailto:${encodeURIComponent(testEmail)}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
    if (state.template.cc) {
      mailtoUrl += `&cc=${encodeURIComponent(state.template.cc)}`;
    }
    if (state.template.bcc) {
      mailtoUrl += `&bcc=${encodeURIComponent(state.template.bcc)}`;
    }
    window.open(mailtoUrl, "_blank");
    addLog("[Test Email] Opened mail client compose drawer.", "success");
    showToast("Test email triggered", "success");
  } else if (state.settings.method === "gmailweb") {
    let gmailUrl = `https://mail.google.com/mail/?view=cm&fs=1&to=${encodeURIComponent(testEmail)}&su=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
    if (state.template.cc) {
      gmailUrl += `&cc=${encodeURIComponent(state.template.cc)}`;
    }
    if (state.template.bcc) {
      gmailUrl += `&bcc=${encodeURIComponent(state.template.bcc)}`;
    }
    window.open(gmailUrl, "_blank");
    addLog("[Test Email] Opened Gmail compose window.", "success");
    showToast("Test email triggered via Gmail", "success");
  } else if (state.settings.method === "brevo") {
    if (!state.settings.brevoKey) {
      addLog("[Test Email Fail] Missing Brevo API key.", "error");
      showToast("Missing API Key", "error");
      return;
    }

    const brevoPayload = {
      sender: { name: state.settings.brevoName || "Personal Sender", email: state.settings.brevoEmail },
      to: [{ email: testEmail, name: "Vesper Test Recipient" }],
      subject: subject,
      htmlContent: body.replace(/\n/g, "<br>")
    };

    if (state.template.cc) {
      brevoPayload.cc = state.template.cc.split(',').map(e => ({ email: e.trim() }));
    }
    if (state.template.bcc) {
      brevoPayload.bcc = state.template.bcc.split(',').map(e => ({ email: e.trim() }));
    }
    ...
    .then(async (response) => {
      if (response.ok) {
        addLog(`[Test Email Success] Verified Brevo API configuration! sent to ${testEmail}`, "success");
        showToast("Test email sent!", "success");
      } else {
        const txt = await response.text();
        addLog(`[Test Email Fail] Brevo server error: ${txt}`, "error");
        showToast("Test email failed", "error");
      }
    })
    .catch(err => {
      addLog(`[Test Email Fail] Network exception: ${err.message}`, "error");
      showToast("Network Error", "error");
    });

  } else if (state.settings.method === "resend") {
    if (!state.settings.resendKey) {
      addLog("[Test Email Fail] Missing Resend API key.", "error");
      showToast("Missing API Key", "error");
      return;
    }
    ...
    if (state.template.cc) {
      resendPayload.cc = state.template.cc.split(',').map(e => e.trim());
    }
    if (state.template.bcc) {
      resendPayload.bcc = state.template.bcc.split(',').map(e => e.trim());
    }
    ...
    .then(async (response) => {
      if (response.ok) {
        addLog(`[Test Email Success] Verified Resend configuration! sent to ${testEmail}`, "success");
        showToast("Test email sent!", "success");
      } else {
        const txt = await response.text();
        addLog(`[Test Email Fail] Resend server error: ${txt}`, "error");
        showToast("Test email failed", "error");
      }
    })
    .catch(err => {
